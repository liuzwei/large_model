...
均方误差（Mean Squared Error, MSE）和平均绝对误差（Mean Absolute Error, MAE）是用于衡量模型预测值与真实值之间差异的常用指标。它们的数值 **越小越好** ，因为它们表示预测误差的大小：
